...
start "LifeLens API Server" cmd /k "cd lifelens && python -m uvicorn api.main:app --host 0.0.0.0 --port 8000"
timeout /t 3 /nobreak > nul

REM Start Streamlit App in background
echo [2/3] Starting Streamlit App...
start "LifeLens Streamlit" cmd /k "cd lifelens && streamlit run app.py"
timeout /t 3 /nobreak > nul

REM Start Medication Scheduler in background
echo [3/3] Starting Medication Scheduler...
start "LifeLens Med Scheduler" cmd /k "python lifelens\scripts\medication_scheduler_service.py"
